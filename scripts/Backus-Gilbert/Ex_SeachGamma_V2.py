...
#%%
Obs_std  = 1.0
MAPAS=['LCA_50000m','LCA_12500m','LCA_25000m']
#MAPAS=['LCA_12500m']
Bands=['KA','KU','K','X','C']
#Bands=['KA']
...
for MAPA in MAPAS:
    ...
    for rep in range(tot_img_samples):
        ...
        for Band in Bands:
            ...
            YI=RMSE_BG(TbV,Context, Observation, gamma=XI,max_obs = max_obs,omega=omega)
            YD=RMSE_BG(TbV,Context, Observation, gamma=XD,max_obs = max_obs,omega=omega)
            YC=RMSE_BG(TbV,Context, Observation, gamma=XC,max_obs = max_obs,omega=omega)
            
            best_gamma,lowest_RMSE, finished_stat = calc_gamma(TbV, Context, Observation, XI,XD,XC,YI,YD,YC,max_obs)
