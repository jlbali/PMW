...
for MAPA in MAPAS:
    ...
    priori_factors=np.exp(np.linspace(-5,5,25))
    ...
    for Band in Bands:
        ...
        RSE=[]
        R=np.zeros([4,l])
        for i,titaa in enumerate(priori_titas):
            tita=copy.deepcopy(titaa)
            L_ParamEst.pr(tita,'Tita entrada')
            ...
            L_ParamEst.pr(titaSol,'Tita Sol')
